...
AmCharts.loadFile = function( url, options, handler ) {
  ...
  if ( window.XMLHttpRequest ) {
	...
	// open the connection
	try{
		request.open( options.type ? options.type : 'GET', options.timestamp ? AmCharts.timestampUrl( url ) : url, options.async );
	}
	catch( e ){
	...
	request.onreadystatechange = function() {
		if ( 4 === request.readyState && 404 === request.status ){
			...
			hideLoading(options.loadingBlock);
  ...
  // load the file
	try {
		...
		showLoading(options.loadingBlock); //Before Send
